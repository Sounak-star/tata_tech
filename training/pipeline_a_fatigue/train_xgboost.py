...
try:
    ...
    _DEVICE = xgb_device()
    print(f"[train_xgboost] compute device: {describe()}")
except Exception:
    _DEVICE = "cpu"

# -- XGBoost hyper-parameters (binary; CUDA when available, else CPU) ---------
XGB_PARAMS = dict(
    objective        = "binary:logistic",
    eval_metric      = "aucpr",
    tree_method      = "hist",        # hist works on both CPU and CUDA
