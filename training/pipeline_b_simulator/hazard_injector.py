...
analysis, handed off as `data/hazard_frequency.csv` (generated by the context
risk model in `context_risk/`). Those are
...
# Dev 3 -> Dev 1 handoff: overall hazard distribution, in the shared data dir.
# Anchored to the project root so it resolves regardless of the caller's CWD.
PROJECT_ROOT = Path(__file__).resolve().parents[2]
DEV3_HAZARD_FREQ = str(PROJECT_ROOT / "data" / "hazard_frequency.csv")
